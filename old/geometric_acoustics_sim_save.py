'''2次元幾何音響シミュレーションプログラム（結果保存付き）
音源から放射される音響粒子の挙動を可視化し、
アニメーションとして動画ファイルに保存し、反射データを記録します
'''
...
def update(frame):
    ...
    if active_balls:
        ...
        scat.set_offsets(np.zeros((0, 2)))  # 2次元の空配列を設定
    return scat,

print('アニメーションを作成中...')
# アニメーション作成と保存
ani = FuncAnimation(fig, update, frames=MAX_STEPS, interval=20, blit=True, repeat=False)
print('アニメーションを保存中...')
ani.save('simulation_animation.mp4', writer='ffmpeg', fps=30)
print('simulation_animation.mp4 に保存しました。')
